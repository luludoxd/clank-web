...
def youtube_error_message(error):
    """Convert common YouTube API errors into useful messages."""
    if isinstance(error, HttpError):
        status = getattr(error.resp, "status", None)
        error_text = str(error)

        if status == 403 and ("quotaExceeded" in error_text or "quota" in error_text.lower()):
            return (
                "YouTube API quota exceeded. Try again later or use another API key."
            )
        if status == 403:
            return (
                "YouTube rejected the API request (403). Check that the YouTube Data API "
                "is enabled and that the API key is valid."
            )
        if status == 400:
            return "YouTube rejected the request (400). Check the YouTube URL."
        if status == 404:
            return "YouTube could not find the requested video or resource."
...
def get_youtube_service(api_key):
    if not api_key or not api_key.strip():
        raise RuntimeError("Please enter a YouTube API key.")
...
def get_comments(api_key, video_id, progress_callback=None):
    """
